...
def train_cpath_bnn(args,
    ...
    for i, (input, target) in enumerate(cpath_train_loader):
        ...
        for mc_run in range(args.num_mc):
            output, kl = model(input_var,clinical_var)
...
def validate_cpath_model(args, cpath_val_loader, model, epoch, tb_writer=None):
...
def evaluate_cpath_model(args, model, cpath_val_loader):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(cpath_val_loader):
            ...
            output_mc = []
            for mc_run in range(args.num_mc):
                output, _ = model.forward(input_var,clinical_var)
                output_mc.append(output)
            output_ = torch.mean(torch.stack(output_mc), dim=0)
            output_list.append(output_)
            print('Test Error:',
                  (partial_ll_loss(output.reshape(-1), tb.reshape(-1), e.reshape(-1)) /args.batch_size
                   ))
        end = time.time()
